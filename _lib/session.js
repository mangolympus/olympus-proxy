...
// treated as routes by Vercel â€” this is just an importable helper, not a public endpoint.
//
...
// library â€” this app only ever needs "is this a real profile, and has it expired," so a
...
export const SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days. Long-lived on purpose â€”
// this is what actually fixes the "logged out constantly" problem Umang was hitting with
// Google's ~1hr access tokens. Re-entering a password once a month is a reasonable ask;
// every hour was not.
...
// Constant-time comparison â€” a plain === leaks timing information proportional to how many
// leading characters match, which is a real (if minor) attack vector for password/token
// checks. timingSafeEqual needs equal-length buffers, hence the length check first (a length
// mismatch is itself safe to reveal via early return â€” it doesn't leak *which* characters
// are right).
...
// This function is the entire access-control boundary for /api/sync â€” every check below
